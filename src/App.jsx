...
function App() {
  ...
  const exportUnsubmittedReport = async () => {
    ...
    } catch (err) { if (err.name !== 'AbortError') alert("存檔失敗"); }
  };
    ...
    <div className="h-screen w-screen bg-[#C3CCD8] flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl p-10 border border-white text-slate-800">
        <form onSubmit={(e) => { e.preventDefault(); if(!formData.schoolName) return alert("請輸入學校"); setIsUploading(true); setDoc(doc(db, "user_profiles", user.uid), { ...formData, email: user.email, displayName: user.displayName, createdAt: Date.now() }).then(() => { setUserProfile(formData); setShowOnboarding(false); }).finally(() => setIsUploading(false)); }} className="space-y-5">
          ...
          <input required type="text" placeholder="學校全稱" value={formData.schoolName} onChange={(e) => setFormData({...formData, schoolName: e.target.value})} className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 font-bold focus:border-blue-500 outline-none transition-all" />
    ...
    <div className="flex h-screen w-screen bg-[#C3CCD8] overflow-hidden font-sans text-slate-800 text-sm">
            ...
            <table className="w-full border-collapse text-slate-800">
                  ...
                  <th className="p-3 w-44 text-center border-r">
                    <div className="flex flex-col items-center gap-1.5">
                       ...
                       <div className="flex gap-2 no-print">
                          <button onClick={() => { if(window.confirm("全交？")) { setIsUploading(true); const b = writeBatch(db); displayStudents.forEach(s => b.update(doc(db, "users", user.uid, "students", s.id), {isDone: true})); b.commit().then(()=>setIsUploading(false)); } }} className="bg-green-600 text-white text-[10px] px-2 py-0.5 rounded shadow-sm font-black tracking-tight text-center">全交</button>
                          <button onClick={() => { if(window.confirm("清空？")) { setIsUploading(true); const b = writeBatch(db); displayStudents.forEach(s => b.update(doc(db, "users", user.uid, "students", s.id), {isDone: false})); b.commit().then(()=>setIsUploading(false)); } }} className="bg-red-600 text-white text-[10px] px-2 py-0.5 rounded shadow-sm font-black tracking-tight text-center">清空</button>
                       </div>
                    </div>
                  </th>
              ...
              <tbody>
                {displayStudents.map((s) => (
                  <tr key={s.id} className="hover:bg-slate-50 border-b border-slate-200 h-11 transition-colors font-bold text-slate-800">
      ...
      <aside className="w-[420px] h-full bg-[#AEB9C8] border-l border-slate-300 p-6 flex flex-col items-center gap-3 shadow-2xl flex-none no-print overflow-hidden text-slate-800">
          ...
          <div className="bg-white border border-slate-400 px-4 py-2.5 rounded-xl shadow-md flex justify-between items-center tracking-tight font-black text-slate-700">
            <div className="flex items-center gap-2 overflow-hidden mr-2">
              ...
              <div className="flex flex-col overflow-hidden leading-tight text-sm">
                ...
                <span className="truncate text-[10px] text-slate-400">{user?.displayName} 老師</span>
              </div>
            </div>
            <button onClick={() => signOut(auth)} className="text-red-500 underline text-xs font-bold tracking-tight">登出</button>
          </div>
          <label className="bg-slate-800 text-white w-full py-3.5 rounded-xl text-center font-black text-lg cursor-pointer hover:bg-black transition-all shadow-lg flex items-center justify-center gap-3 active:scale-95">
            <Upload size={22}/> 匯入新回條名單 <input type="file" onChange={handleFileUpload} className="hidden" />
          </label>
          <button onClick={exportUnsubmittedReport} className="bg-white border-2 border-indigo-800 w-full py-3.5 rounded-xl text-center font-black text-lg text-indigo-900 flex items-center justify-center gap-3 shadow-md hover:bg-indigo-50 active:scale-95">
        ...
        <div className="flex items-center gap-2 text-slate-700 bg-white/40 px-4 py-1.5 rounded-full border border-slate-500 font-black shadow-inner mt-1">
          <Eye size={14} />
          <span className="text-[10px] tracking-widest uppercase text-slate-600 font-black tracking-tight">Traffic: {visitorCount}</span>
        </div>
        ...
        <div className="w-full space-y-2.5 px-4 flex-none font-bold text-sm">
          <div className="border border-slate-600 p-3 rounded-xl bg-white/30 flex justify-between items-center shadow-md border-l-8 border-l-emerald-500 transition-all"><span className="text-md font-black tracking-tight text-slate-800">已繳交：</span><span className="text-2xl font-black">{doneCount}</span></div>
          <div className="border border-slate-600 p-3 rounded-xl bg-white/30 flex justify-between items-center shadow-md border-l-8 border-l-red-500 text-red-800 tracking-tighter font-black"><span className="text-md">未繳交：</span><span className="text-2xl font-black">{total - doneCount}</span></div>
        </div>
        ...
        <div className="w-full mt-auto flex flex-col items-center">
          <div className="bg-[#5B6B7E]/25 p-4 rounded-2xl border border-white/20 w-full shadow-inner text-slate-800 backdrop-blur-sm">
            <div className="flex flex-row items-center gap-4 w-full">
               <div className="flex-none"><img src="/line-qr.png" alt="Line QR" className="w-[80px] h-[80px] rounded-lg shadow-md mix-blend-multiply opacity-95 grayscale-[10%]" style={{ filter: 'brightness(0.95) contrast(1.1)' }} /></div>
               <div className="flex-grow flex flex-col gap-1.5 overflow-hidden">
                  ...
                  <div className="flex flex-col gap-1 mt-0.5">
                    <div className="flex items-center gap-1.5 text-[10px] font-black text-indigo-950 bg-white/40 px-2.5 py-1 rounded-full w-fit shadow-sm truncate"><Mail size={11} className="text-slate-500 flex-none"/> yaoink@gmail.com</div>
                    <div className="flex items-center gap-1.5 text-[10px] font-black text-indigo-950 bg-white/40 px-2.5 py-1 rounded-full w-fit shadow-sm truncate"><Coffee size={11} className="text-slate-500 flex-none"/> Line ID：@056hkncr</div>
                  </div>
               </div>
            </div>
            <p className="text-[8px] text-slate-500 font-bold opacity-50 uppercase mt-2 text-center tracking-widest border-t border-slate-500/10 pt-1">Education System v7.4 | © 2025</p>
          </div>
        </div>
      </aside>
      ...
      <div style={{ position: 'absolute', left: '-9999px', top: '0' }}>
        <div ref={reportRef} style={{ width: '800px', padding: '50px', backgroundColor: '#ffffff', fontFamily: 'sans-serif' }}>
          ...
          {Object.keys(groupedUnsubmitted).length > 0 ? (
            Object.keys(groupedUnsubmitted).sort().map(className => (
            ...
            <p style={{ textAlign: 'center', fontSize: '24px', color: '#10b981', fontWeight: 'bold', marginTop: '50px' }}>恭喜！全部人員皆已繳齊。</p>
          )}
        </div>
      </div>
      ...
      {isUploading && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-md flex flex-col items-center justify-center text-white font-bold p-8 text-center">
           ...
           <p className="text-xl font-black tracking-widest uppercase italic">Syncing Cloud...</p>
        </div>
      )}
    </div>
  );
